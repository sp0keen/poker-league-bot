...
function computeBlindLevels(denoms) {
  ...
  for (let i = 0; i < maxLevels; i++) {
    ...
    // дальше блайнд просто плавно удваивается — независимо от того, каким конкретно физическим
    // номиналом его потом обозначат (это отдельный вопрос, см. activeDenomsAtLevel). Раньше здесь
    // шаг подгонялся под круглое переобозначение самого мелкого номинала (×10/×100/...), из-за
    // чего рост мог внезапно скакнуть в 5+ раз за один уровень вместо привычного удвоения —
    // тем резче, чем меньше номиналов в наборе (якорное удвоение "используется" раньше)
    else sb *= 2;
